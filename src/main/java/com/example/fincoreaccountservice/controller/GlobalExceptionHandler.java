...
import com.example.fincoreaccountservice.exception.CurrencyNotFoundException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDto> handleDuplicateBankBookException(DuplicateBankBookException exception) {
        return ResponseEntity.badRequest().body(new ErrorDto(exception.getMessage(), "DUPLICATE_ACCOUNT"));
    }
    ...
    public ResponseEntity<ErrorDto> handleCurrencyNotFoundException(CurrencyNotFoundException exception) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ErrorDto(exception.getMessage(),
                "CURRENCY_NOT_FOUND"));
    }
}
